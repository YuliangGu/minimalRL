...
Two networks share the same architecture:

- **Target network** :math:`\hat{f}_{\theta}(x)` is randomly initialized and frozen; it maps observations to a feature embedding.
- **Predictor network** :math:`f_{\phi}(x)` is trained to regress onto the target's features.

Intrinsic Reward
----------------

During rollouts, the agent receives an intrinsic reward proportional to the prediction error, often scaled and normalized:

.. math::

   r^{\text{int}}_t \propto \lVert f_{\phi}(x_t) - \hat{f}_{\theta}(x_t) \rVert_2^2.
